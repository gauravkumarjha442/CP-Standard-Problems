...
int max_in_range(int v, int tl, int tr, int l, int r) {
    if (l > r) 
        return 0;
    if (l == tl && r == tr) {
        return t[v];
    ...
    int tm = (tl + tr) / 2;
    return max(max_in_range(v*2, tl, tm, l, min(r, tm))
             , max_in_range(v*2+1, tm+1, tr, max(l, tm+1), r));
}
...
void solve()
{
    ...
    cout<<max_in_range(1 , 0 , n-1 , 0 , 3)<<endl;

    update(1,0,n-1,3,0);

    cout<<max_in_range(1 , 0 , n-1 , 0 , 3);
